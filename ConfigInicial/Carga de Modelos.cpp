...
int main( )
{
    ...
    GLFWwindow *window = glfwCreateWindow( WIDTH, HEIGHT, "Jeremias Sanchez. Carga de modelos y camara sintetica", nullptr, nullptr );
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // segundo modelo del perro
        model = glm::translate(model, glm::vec3(-0.5f, 0.4f, 0.5f));
        model = glm::scale(model, glm::vec3(3.0f, 3.0f, 3.0f));
        glUniformMatrix4fv(glGetUniformLocation(shader.Program, "model"), 1, GL_FALSE, glm::value_ptr(model));
        dog.Draw(shader);

        // tercer modelo, rama
        model = glm::translate(model, glm::vec3(0.5f, 0.1f, 0.5f)); // Se posiciona
