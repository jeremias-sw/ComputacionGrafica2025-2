...
// Window dimensions
const GLuint WIDTH = 2680, HEIGHT = 1540;
...
int main()
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Jeremias Sanchez. Skybox", nullptr, nullptr);
...
void KeyCallback(GLFWwindow *window, int key, int scancode, int action, int mode)
{
	...
	if (keys[GLFW_KEY_B]) // Animación de caminar
	{
		dogAnim = 1;
...
void Animation() {
	...
	if (dogAnim == 1) {		// máquina de estados	
		if (dogPos.z < 2.35f) {		// Animación de caminar
			if (!step) {			// estado 1
				RLegs += 0.07f;
				FLegs += 0.07f;
				head += 0.07f;
				...
				if (RLegs < -15.0f)	// condicional
					step = false;
			}

			dogPos.z += 0.0007;
		}
		else {						// estado 0 de reposo (idle)
			RLegs = 0.0f;
			FLegs = 0.0f;
			head = 0.0f;
			tail = 0.0f;
